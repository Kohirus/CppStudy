...
struct ngx_pool_data_t {
    ...
    ngx_pool_t* next;   // 指向下一个小块内存
    ngx_unit_t  failed; // 当前小块内存池分配内存失败的次数
};
...
struct ngx_pool_s {
    ...
    ngx_pool_cleanup_t* cleanup; // 指向预置清理操作的回调函数入口
};
